...
#### 次时代 RAG 数据库架构：SQLite + USearch 全面重构

VCP 5.1 的记忆系统经历了一次**革命性的底层重构**，从 PostgreSQL + pgvector 架构全面迁移到更适合个人化部署的 **SQLite + USearch** 架构。这是基于 VCP 定位于个人 AI 环境的战略性技术选型，用 Rust 重写了所有性能关键路径。
...
- **极致的可移植性**：整个记忆库就是一个 `.db` 文件，备份、迁移、版本控制都极其简单
...
### Tag 向量网络牵引检索 (Tag Vector Network Retrieval) - VCP 原创算法

这是完全原创的、开创性的检索增强技术，在传统 RAG 基础上引入了全新的 "Tag 向量牵引层"，通过**三阶段透镜-拓展-聚焦**机制，从根本上提升知识库召回的精准度。

**启用语法**：`[[角色日记本::Tag0.65]]` 或 `《《知识库::Tag0.85::Rerank》》`
