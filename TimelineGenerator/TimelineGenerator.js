...
async function processSummaryQueue() {
    ...
    while (summaryQueue.length > 0) {
        ...
        // 1. 根据文件内容（第一行）中的角色名对文件进行分组，以实现智能并发
        ...
        for (const filePath of batch) {
            try {
                ...
                const characterName = (match && (match[2] || match[4])) ? (match[2] || match[4]).trim() : 'unknown';
                
                if (!characterGroups[characterName]) {
                    characterGroups[characterName] = [];
                }
                characterGroups[characterName].push(filePath);
            } catch (e) {
                console.warn(`[TimelineGenerator] Could not pre-read file ${path.basename(filePath)} for grouping, placing in fallback group.`, e);
                if (!characterGroups['unknown']) characterGroups['unknown'] = [];
                characterGroups['unknown'].push(filePath);
            }
        }

        // 2. 并行处理不同角色的文件组，但组内的文件按顺序处理以避免锁冲突
        const groupProcessingPromises = Object.values(characterGroups).map(async (fileGroup) => {
            for (const filePath of fileGroup) {
                try {
                    await processFile(filePath);
                } catch (e) {
                    // processFile 内部已有更详细的错误处理，这里作为最后防线
                    console.error(`[TimelineGenerator] Unhandled error during sequential group processing for ${path.basename(filePath)}:`, e);
                }
            }
        });
...
async function processFile(filePath) {
    try {
        ...
        if (existingEntry && existingEntry.status === 'fallback') {
            const timelineFilePath = path.join(tvsTxtDir, `${characterName}timeline.txt`);
            try {
        ...
        if (error.code === 'ELOCKED') {
            console.warn(`[TimelineGenerator] File lock failed for ${path.basename(filePath)}. Re-queuing for a later attempt.`);
            // Re-queue the file after a short delay to prevent busy-looping
            setTimeout(() => addToQueue(filePath), 2000);
        } else {
