...
module.exports = function(DEBUG_MODE, dailyNoteRootPath, pluginManager, getCurrentServerLogPath, vectorDBManager) {
    ...
    const TVS_DIR = path.join(PROJECT_BASE_PATH, 'TVStxt');

    // 确保目录存在
    fs.mkdir(TOOL_CONFIGS_DIR, { recursive: true }).catch(err => {
        console.error('Failed to create ToolConfigs directory:', err);
    });

    // 获取所有可用工具
    adminApiRouter.get('/tool-list-editor/tools', async (req, res) => {
        try {
            const allTools = [];
            ...
            console.error('[Tool List Editor] Failed to get tools:', error);
    ...
    adminApiRouter.get('/tool-list-editor/configs', async (req, res) => {
        try {
            ...
            console.error('[Tool List Editor] Failed to get configs:', error);
            res.json({ success: true, configs: [] });
        }
    });

    // 加载配置文件
    adminApiRouter.get('/tool-list-editor/config/:name', async (req, res) => {
        try {
            ...
            console.error('[Tool List Editor] Failed to load config:', error);
            res.status(500).json({ success: false, error: error.message });
        }
    });

    // 保存配置文件
    adminApiRouter.post('/tool-list-editor/config/:name', async (req, res) => {
        try {
            const configName = req.params.name;
            const configPath = path.join(TOOL_CONFIGS_DIR, `${configName}.json`);
            ...
            console.error('[Tool List Editor] Failed to save config:', error);
            res.status(500).json({ success: false, error: error.message });
        }
    });

    // 删除配置文件
    adminApiRouter.delete('/tool-list-editor/config/:name', async (req, res) => {
        try {
            const configName = req.params.name;
            const configPath = path.join(TOOL_CONFIGS_DIR, `${configName}.json`);
            ...
            console.error('[Tool List Editor] Failed to delete config:', error);
            res.status(500).json({ success: false, error: error.message });
        }
    });
    ...
    adminApiRouter.post('/tool-list-editor/export/:fileName', async (req, res) => {
        try {
            ...
            const txtPath = path.join(TVS_DIR, `${fileName}.txt`);
            await fs.writeFile(txtPath, output, 'utf8');
            
            res.json({ success: true, filePath: txtPath });
        } catch (error) {
            console.error('[Tool List Editor] Failed to export:', error);
            res.status(500).json({ success: false, error: error.message });
        }
    });
    // ==================== End Tool List Editor API ====================
